...
//LinkedList.cpp
//Project 5 - Made helper functions to sort the feed from the network
****************************************************************************************************************************/
...
void LinkedList<ItemType>::bubbleSort(Comparator compare, int* count){
                Node<ItemType>* curr_item = head_; //set iterator at beginning of feed

                int size = size_;
                bool swapped = true; // assume unsorted
                int pass = 1;
                while(swapped && (pass < size)) //while unsorted and not at end of feed
                {
                    swapped = false; 
                    curr_item = head_; //resets the bubble sort
                    for(int i = 0; i < size - pass; i++){
                        if(compare( *(curr_item->getNext()->getItem()), *(curr_item->getItem()))){ //the star derefrences the pointer
                            swapNodes(*(curr_item), *(curr_item->getNext())); //drefrence to get a node
                            (*count)++; //increase swap count
                            swapped = true; //feed not sorted

                        }
                     curr_item = curr_item->getNext(); //iterates to next node
            ...
            //head is pointer to first node of the list
            //head is the first item of the feed in Network
    ...
    template<typename ItemType>
        void LinkedList<ItemType>::swapNodes(Node<ItemType> &item, Node<ItemType> &item2){ //helper function to swap the node
        Node<ItemType> *node = new Node<ItemType>(); //temp node to swap the values
        node->setItem(item.getItem());
        item.setItem(item2.getItem());
        item2.setItem(node->getItem());
        delete node; //deallocate the temp node
        node = nullptr;


    
             
            //swaps just the values? 
            // item->getItem() = item2->getItem();
            // item2->getItem() = temp->getItem();
            // delete temp;
        }
